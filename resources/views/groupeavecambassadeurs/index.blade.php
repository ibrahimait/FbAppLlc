...
@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h2>Liste des groupes avec ambassadeurs</h2>
            </div>
        ...
        <div class="row">
                <div class="col-lg-40">
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-md-2 col-sm-30 p-0">
                            ...
                            <button type="button" class="btn btn-danger wrn-btn"><a href="{{ route('groupeavecambassadeurs') }}">Reset</a></button>
                        </div>
                ...
                <div  style="margin-top : 20px">
                        ...
                        <select data-trigger="" name="nm" style="width : 50px"  onchange="this.form.submit();">
                            <option disabled selected>{{$groupeavecambassadeurs->perPage()}} </option>
                            <option>10</option>
                            <option>25</option>
                            <option>50</option>
                            <option>100</option>
...
<!-- Recherche Multicritères FIN -->
            ...
            <br>
                <a class="btn btn-success" href="{{ route('groupeavecambassadeurs.create') }}"> Ajouter</a>
                <a class="btn btn-info" href="{{ route('groupes.index') }}"> Retour</a>
            </div>
            <p> Il y'a : {{ $groupeavecambassadeurs->total() }} Groupes</p>
        </div>
    </div>
   <br><br>
    ...
    <div class="table">
        <table class="table">
            ...
            @if($groupeavecambassadeurs->total())
            @foreach ($groupeavecambassadeurs as $key => $value)
             <tr>
                <th  scope="row">{{++$i}}</th>
                ...
                <td>
                    <form action="{{ route('groupesansambassadeurs.destroy',$value->id) }}" method="POST">
                    <a style="background-color : #fafafa ; border : #fafafa"
                     class="btn btn-info" href="{{ route('groupes.show',$value->id) }}">
                    <i style="color : black" class="fas fa-info"></i></a>
                    ...
                    <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                        ...
                        @csrf
                        @method('DELETE')
                        <button  style="background-color : #fafafa ; border : #fafafa"  type="submit"
                         class="btn btn-danger"><i style="color : black"   class="fas fa-trash"></i></button>
                    </form>
                </td>
            </tr>
            @endforeach
