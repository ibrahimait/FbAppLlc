...
@section('content')
    ...
    <div class="table">
        ...
        <form method="post">
            ...
            <a class="btn btn-success" href="{{ route('clients.create') }}" style="margin-bottom : 14px "> Ajouter</a>
            <br>
            ...
            <table class="table">
                <thead class="thead" style="background:#cccccc">
                    <tr>
                        ...
                        <th>@sortablelink('id_stripe')</th>
                        <th>@sortablelink('url_stripe')</th>
                        <th>Action</th>
                    </tr>
                </thead>
                @if ($clients->count())
             ...
             <td> <a href="{{ \Str::limit($value->id_contact)}}"target="_blank">{{ $value->id_contact }}</a></td>
                            ...
                            <td>{{ $value->tel }}</td>
                            <td>{{ $value->email }}</td>
                            <td>{{ $value->id_client_stripe }}</td>
                            <td>{{ $value->url_client_stripe }}</td>
                            <td>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                    href=""><i style="color : #4d94ff"
                                        class="fas fa-check"></i></a>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                    href="{{ route('clients.show', $value->id_contact) }}"><i style="color : #4d94ff"
                                        class="fas fa-info"></i></a>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-info"
                                    href=""><i style="color : #4d94ff"
                                        class="fas fa-eye"></i></a>
                                <a style="background-color : #fff ; border : #fff" class="btn btn-primary"
                                    href="{{ route('clients.edit', $value->id_contact) }}"><i style="color : #e6ac00"
                                        class="fas fa-edit"></i></a>
                                <button formaction="{{ route('clients.destroy', $value->id_contact) }}"
